...
class GameLibraryDialog(QDialog):
    ...
    def _show_game_details_dialog(self, game_data):
        ...
        def _open_folder():
            ...
            try:
                ...
                if system == "windows":
                    subprocess.run(['explorer', str(install_path)], check=False)
                elif system == "darwin":  # macOS
                    subprocess.run(['open', str(install_path)], check=False)
                else:  # Linux and other Unix-like
                    subprocess.run(['xdg-open', str(install_path)], check=False)
            except Exception as e:
                QMessageBox.warning(
                    self,
                    "Open Folder",
        ...
        if sgdb_api_key and platform.system() == "Linux":
        ...
        apply_goldberg_btn = QPushButton("Apply Goldberg")
        apply_goldberg_btn.setToolTip(
            "Apply Goldberg files to the game folder to play it without using Steam client."
        )
        apply_goldberg_btn.clicked.connect(
            lambda checked, dir=install_path, aid=appid, name=game_data.get("game_name", "Unknown"):
            self.main_window.task_manager.apply_goldberg_to_game(dir, aid, name, show_dialog=True)
        )
    ...
    def _fetch_game_manifest(self, game_data, dialog):
        """Package local manifest and add to job queue using ManifestPackager"""
        ...
        if not app_id or app_id == "0":
            QMessageBox.warning(
                self,
                "Invalid App ID",
                f"Cannot fetch manifest: App ID is invalid or missing for '{game_data.get('game_name', 'Unknown')}'.",
            )
            return
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Download",
            f"Package and download manifest for '{game_name}' (App ID: {app_id})?\n\n"
            f"The manifest will be packaged from local files and added to the queue.\n"
        ...
        # Show progress dialog while packaging
        progress = QProgressDialog(
            f"Packaging manifest for {game_name}...", "Cancel", 0, 0, self
        )
        ...
        try:
            # Use ManifestPackager to build ZIP from local Lua + manifest files
            zip_path, error_msg = ManifestPackager().package(str(app_id))

            if zip_path:
                # Success — pass the ZIP to the existing pipeline
                progress.close()
                ...
                self.main_window.job_queue.add_job(zip_path, metadata)
                ...
                # Packaging failed
                progress.close()
                logger.error(f"ManifestPackager failed for AppID {app_id}: {error_msg}")
                QMessageBox.critical(
                    self,
                    "Packaging Failed",
                    f"Failed to package manifest for '{game_name}' (App ID: {app_id}):\n\n{error_msg}",
                )

        except Exception as e:
            # Unexpected exception
            progress.close()
            logger.exception(f"Error packaging manifest for AppID {app_id}: {e}")
            QMessageBox.critical(
                self, "Error", f"An error occurred while packaging manifest:\n\n{str(e)}"
            )
    ...
    def _uninstall_game(self, game_data, dialog):
        ...
        success, error_msg = self.game_manager.uninstall_game(
            game_data, remove_compatdata=remove_compatdata, remove_saves=remove_saves, remove_from_library=remove_from_library
        )
