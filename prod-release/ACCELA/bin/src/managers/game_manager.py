...
from core.steam_helpers import get_steam_libraries, get_library_index
...
class GameManager(QObject):
    ...
    def _sync_app_tokens_from_manifests(self):
        """
        Scan $SteamRoot/config/stplug-in/*.lua for app tokens
        ...
        # Locate SteamRoot via get_steam_libraries()
        steam_libraries = get_steam_libraries()
        if not steam_libraries:
            logger.warning("Cannot locate Steam root for token sync: no Steam libraries found")
            return

        steam_root = steam_libraries[0]
        stplug_in_dir = Path(steam_root) / "config" / "stplug-in"
        if not stplug_in_dir.exists():
            logger.debug(f"stplug-in directory not found at {stplug_in_dir}, skipping token sync")
            return

        # Get existing tokens from config to avoid duplicates
        existing_tokens = get_app_tokens(config_path)
        logger.debug(f"Found {len(existing_tokens)} existing AppTokens in config")

        # Regex patterns for extracting AppID and token from Lua files
        appid_pattern = re.compile(r'addappid\((\d+)', re.IGNORECASE)
        token_pattern = re.compile(r'addtoken\((\d+)', re.IGNORECASE)

        tokens_added = 0
        tokens_skipped = 0

        try:
            for lua_file in stplug_in_dir.glob("*.lua"):
                try:
                    lua_content = lua_file.read_text(encoding="utf-8", errors="replace")

                    # Extract AppID from first addappid(...) call
                    appid_match = appid_pattern.search(lua_content)
                    if not appid_match:
                        logger.warning(f"Malformed Lua plugin at {lua_file}: no addappid entry found, skipping")
                        continue
                    ...
                    # Extract token via addtoken(...) regex
                    token_match = token_pattern.search(lua_content)
                    if not token_match:
                        # No token in this Lua file — not an error, just skip
                        continue
                    ...
                    if add_app_token(config_path, app_id, app_token):
                        tokens_added += 1
                        logger.info(f"Added missing AppToken for AppID {app_id} from {lua_file.name}")
                    else:
                        tokens_skipped += 1

                except Exception as e:
                    logger.warning(f"Failed to process Lua plugin {lua_file.name}: {e}")
                    continue

        except Exception as e:
            logger.error(f"Error scanning stplug-in directory for tokens: {e}", exc_info=True)
            return

        if tokens_added > 0:
            logger.info(f"Synced {tokens_added} missing AppToken(s) from stplug-in Lua plugins")
        if tokens_skipped > 0:
    ...
    def _collect_game_data(self, game_path, game_name, library_path):
        ...
        try:
            ...
            game_data = {
                ...
                "library_index": get_library_index(library_path),
    ...
    def uninstall_game(self, game_data, remove_compatdata=False, remove_saves=False, remove_from_library=False):
        """
