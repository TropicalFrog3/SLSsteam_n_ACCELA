...
from utils.helpers import ensure_dotnet_availability, get_dotnet_path, resource_path
...
class DownloadDepotsTask(QObject):
    ...
    def _prepare_downloads(self, game_data, selected_depots, dest_path):
        ...
        dll_path = resource_path("deps/DepotDownloaderMod.dll").absolute()
        ...
        for depot_id in selected_depots:
            ...
            # Only include -manifestfile if the binary manifest exists and has content
            # (placeholder files from API fallback are empty — DepotDownloaderMod will
            # fetch the manifest from Steam's CDN using just the manifest ID)
            if os.path.isfile(manifest_file_path) and os.path.getsize(manifest_file_path) > 0:
                cmd.extend(["-manifestfile", manifest_file_path])
            ...
            accela_username = os.environ.get("ACCELA_USERNAME")
            if accela_username:
                cmd.extend(["-username", accela_username])
