...
from core.steam_helpers import find_steam_install, fix_greenluma_offline_mode, get_steam_libraries
...
def _get_terminal_command(appid=None, zip_path=None):
    ...
    Args:
        appid: AppID (passed through to CLI, ZIP is pre-built by helper script)
...
def open_cli_terminal(appid=None, zip_path=None):
    ...
    Args:
        appid: AppID (ZIP is pre-built by helper script before CLI opens)
        zip_path: Path to a ZIP file to process
    """
...
def run_cli_mode(app, command_line_zips, logger, appid=None):
    ...
    Args:
        ...
        appid: Optional AppID (no-op; ZIP is pre-built by helper script upstream)
    """
...
class CLITaskManager:
    ...
    def _move_manifests_to_depotcache(self):
        ...
        if not steam_path:
            self.logger.error("Cannot locate Steam root for depotcache placement")
            return
        target_depotcache_dir = os.path.join(steam_path, "config", "depotcache")
    ...
    def _add_appids_to_slssteam_config(self):
        ...
        try:
            ...
            # NEW: Add app token to config (Req 9 AC2)
            app_token = self.game_data.get("app_token")
            if main_appid and app_token:
                add_app_token(config_path, str(main_appid), app_token)
                self.logger.info(f"Added AppToken for AppID '{main_appid}' to SLSsteam config")
    ...
    def _app_id_exists_in_applist(app_list_dir, app_id_to_check):
        ...
        try:
            ...
            # Can't use self.logger in static method
            pass
